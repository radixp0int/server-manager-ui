...
export class ServerService {
  private readonly apiUrl: string = 'http://localhost:8080/api/v1';
          ...
          status === Status.ALL ? { ...response, message: `Server filtered by ${status} status` } :
            {
              ...
              message: response.data.servers
                ...
                `Server filtered by ${status === Status.SERVER_UP ?
                  'SERVER UP' : 'SERVER DOWN'} status` : `No Servers of ${status} found!`,
              data: {
